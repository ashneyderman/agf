...
class WorkflowTaskHandler:
    ...
    def _run_prompt(self, worktree: Worktree, task: Task) -> str:
        """Execute a prompt directly with the agent and return the output.
        ...
        Returns:
            Agent output from executing the prompt
        ...
        # Execute agent with prompt directly
        self._log(f"Running prompt with agent {effective_agent} and model {model}")
        ...
        self._log(
            f"Prompt execution completed: success={result.success}, exit_code={result.exit_code}"
        )
        self._log(f"output={result.output}")
